...
function getproinfo() {
    ...
    $.post("/Home/getProductinfo", { id: productID })
        ...
        .done(function (result) {
            ...
            $(".this-pprise").html(result.productinfo.price + `<del class="product-old-price">${(result.productinfo.price) - ((result.productinfo.price / 100) * result.productinfo.discount)}</del>`)
            ...
            for (image in result.productimg[0]) {
                console.log(image)
                if (result.productimg[0][image] != null) {


                    $("#product-imgs").append(`
                 <div class="product-preview">
                        <img src="../../assets/img/products_imgae/${result.productimg[0][image]}" alt="">
                    </div>
                      `);
